	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			if(menu ==0) {
				...
				System.out.println("메뉴선택 오류. 종료는 0");
			continue;
			}for(;;) {
				//for(;;) 는 무한루프											
				System.out.println("계산할 값을 입력해주십시오. (계산 종료:0 , 전체 종료:99) > ");
				tmp = scanner.next();
				...
				if(num ==0) {
					...
					if(num==99) {
						break outer; //전체 종료. 바깥쪽 while문까지 모두벗어남.
					}
					switch (menu) {
					case 1:
					...
					case 2:
						...
						break;
					case 3:
						System.out.println("result = " +Math.log(num)); //로
						break;
